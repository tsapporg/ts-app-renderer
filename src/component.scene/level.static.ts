// This file is responsible for defining our static level example.
// The level is implemented in all supported renderers.
// TODO chrome only renders this scene sometimes for babylon. 
import * as core from 'ts-app-core';
core.logger.debug('level.static loaded');
...
import * as urls from './filament.urls';

@core.classLogger.LogClass()
export default class ExampleStaticLevel extends ALevel {
  private boxSize = 1;
  ...
  constructor(scene: IScene, onLoad?: CallableFunction) { 
    ...
    if (this.scene.getRenderer().getConfig().rendererLibrary === 'filament') {
      this.initialAssets = [
        urls.skySmall,
        urls.ibl,
        urls.pbrMaterial,
  ...
  async trySaveFrameAsPNG() {
    if (!this.pixels) { console.log('TODO'); }

    (this.scene as ExampleScene).saveSceneFrame(this.pixels!, this.scene.getRenderer().getConfig().exportConfig);
  }
}